...
public class CustomIndicatorManager {
    ...
    /**
     * Discovers indicators provided by resource packs by scanning textures/gui/indicators/ in all namespaces.
     */
    private static void discoverResourcePackIndicators() {
        ...
        try {
            ...
            // Scan all resources under textures/gui/indicators/ across all namespaces
            Map<ResourceLocation, Resource> resources = resourceManager.listResources(
                    "textures/gui/indicators",
                    loc -> loc.getPath().endsWith(".png")
            );

            for (ResourceLocation location : resources.keySet()) {
                String path = location.getPath(); // e.g. "textures/gui/indicators/foo.png"
                String filename = path.substring(path.lastIndexOf('/') + 1); // "foo.png"
                String name = filename.substring(0, filename.lastIndexOf('.')); // "foo"
